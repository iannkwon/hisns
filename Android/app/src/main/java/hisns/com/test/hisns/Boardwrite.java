...
public class Boardwrite extends AppCompatActivity {
    ...
    String wdate;
    String unum;
    String link1="http://192.168.0.121:8090/SnsGo/Boardwrite.do";
...
//        unum = String.valueOf(42);//닉네임
        wdate=String.valueOf(df.format(sd));//날짜
    ...
    private void insertGo(){
        class InsertData extends AsyncTask<String, Void, String> {
            ...
            protected void onPreExecute(){
                loading = ProgressDialog.show(Boardwrite.this, "작성중....", null, true, true);
                super.onPreExecute();
            }
            @Override
...
//            SessionNow.setSession(this,"token1", re1);
            Intent in=new Intent(Boardwrite.this,MainPage.class);
            startActivity(in);
            finish();
        }
    }
...
//            fileName = cursor.getString(cursor.getColumnIndex("bucket_display_name"));
            Date sd = new Date(System.currentTimeMillis());//날짜 가져오기
            SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
            fileName = "/"+String.valueOf(df2.format(sd))+cursor.getString(cursor.getColumnIndex("_display_name"));
            cursor.moveToNext();
        }
        cursor.close();
